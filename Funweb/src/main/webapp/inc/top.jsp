...
<%
// 세션 객체에 저장된 아이디(sId 속성)를 가져와서 id 변수에 저장
// EL과 JSTL 사용시 불필요
...
<header>
	...
	<div id="login">
		...
		<!-- 세션 아이디가 null이면 미로그인, 아니면 로그인 상태 -->
		...
		<!-- 만약, 세션 아이디가 "admin"일 경우 "관리자페이지" 링크 표시(/member/admin.jsp) -->
		<!-- 문자열은 == 안쓰고 equals 씀 -->
	
		<% if (id.equals("admin")) { %>
		 | <a href="../member/admin.jsp">관리자페이지</a>
		<%
		}
		%>
		<%
		...
		<!-- if~else 문 대신 c:choose, when, otherwise 태그 활용 -->
		<!-- EL 사용하여 세션 객체에 접근하기 위해서는 sessionScope.속성명으로 접근필요 -->
		<%-- 	<c:choose>
