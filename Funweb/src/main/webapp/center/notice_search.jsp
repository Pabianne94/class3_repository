...
int pageLimit = 10; // 한 화면에서 표시할 페이지 갯수

//BoardDAO 객체의 selectListCount 메소드를 호출하여 게시물 전체 목록 조회 작업 요청
//>> 파라미터 : 검색어(search), 검색타입(searchType), 리턴타입 : int(listCount)
BoardDAO dao = new BoardDAO();
int listCount = dao.selectSearchListCount(search, searchType);
...
//3. 현재 페이지에서 보여줄 끝 페이지 번호(10, 20 ,30) 계산
...
//BoardDAO 객체의 searchBoardList 메소드를 호출하여 게시물 목록 조회 작업 요청
//>> 파라미터 : 현재 페이지 번호(pageNum), 표시할 목록 갯수(listLimit), 검색어(search)
//>> 리턴타입 : ArrayList<BoardBean>(boardList)
ArrayList<BoardBean> boardList = dao.selectSearchBoardList(pageNum, listLimit, search, searchType);
%>
<!DOCTYPE html>
<html>
<head>
	...
	<div id="wrap">
		...
		<article>
			...
			<table id="notice">
				...
				<%
				// 1. ArrayList 크기만큼 일반 for문으로 반복하면서 BoardBean 꺼내기(형변환 필요)
				...
				<tr 
				onclick="location.href='notice_content.jsp?num=<%=board.getNum() %>&page=<%=pageNum %>'">
					<td><%=board.getNum()%></td>
			...
			<div id="table_search">
				<form action="notice_search.jsp" method="get">
					<select name="searchType">
						<!-- searchType이 name일 경우 작성자 항목을 선택 -->
						<option value="subject">제목</option>
						<option value="name" 
						<%if (searchType.equals("name")) {%> selected <%}%>>작성자</option>
					</select>
					<!-- 검색 기능 -->
			...
			<div class="clear"></div>
			<div id="page_control">
			<!-- 페이징 처리 시 링크에 notice.jsp 대신 notice_search.jsp로 이동(검색어도 전달) -->
				<!-- 
			현재 페이지 번호(pageNum)가 1보다 클 경우에만 prev 링크 동작
			>> 클릭 시 notice.jsp로 이동하면서 현재 페이지 번호(pageNum) - 1 값을 page 파라미터로 전달
			 -->
				<%
				if (pageNum > 1) { // 이전페이지가 존재할 경우
				%>
				<a href="notice_search.jsp?
				page=<%=pageNum - 1%>&search=<%=search %>&searchType=<%=searchType%>">Prev</a>
				...
				<!-- 페이지 번호 목록은 시작 페이지 번호(startPage부터 끝 페이지(endPage) 까지 표시 -->
				<%
				...
				%>
				<a href="notice_search.jsp?
				page=<%=i%>&search=<%=search %>&searchType=<%=searchType%>"><%=i%></a>
				<%
				}
				}
				%>
				...
				<!-- 
			현재 페이지 번호(pageNum)가 끝 페이지 번호보다 작을 때에만 Next 링크 동작
			>> 클릭 시 notice.jsp로 이동하면서 현재 페이지 번호(pageNum) + 1 값을 page 파라미터로 전달
			 -->
				...
				%>
				<a href="notice_search.jsp?
				page=<%=pageNum + 1%>&search=<%=search %>&searchType=<%=searchType%>">Next</a>
				<%
				} else {
				%>
				&nbsp;Next
