...
public class MemberDAO {
	...
	public int updateMember(MemberBean member) {
		...
		try {
			...
			// 단, pass값이 입력되어 있을 경우 패스워드도 수정하고
			// 아니면 이름, 이메일, 주소, 전화번호, 폰번호만 수정

			if (member.getPass().equals("")) { // 패스워드 미 입력시
				...
				String sql = "UPDATE member "
						+ "SET pass=?, name=?, email=?, address=?, phone=?, mobile=? WHERE id=?";
				pstmt = con.prepareStatement(sql);
	...
	public boolean checkId(String id) {
		...
		try {
			...
			// 조회된 레코드(아이디)가 존재할 경우 아이디가 중복이므로 isDuplicate를 false로 변경
			if (rs.next()) {
				isDuplicate = true;
			}
		} catch (SQLException e) {
